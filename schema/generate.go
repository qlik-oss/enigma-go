...
func generateVersionFile(schema *Schema, version, pwd string) {
	if !isValidVersion(version) {
		fmt.Println("Invalid version, should be of the from d.d.d")
		os.Exit(1)
	}
	out, err := os.Create(pwd + "/../version.go")
	defer out.Close()
	if err != nil {
		fmt.Println(err.Error())
		os.Exit(1)
	}
	qixVersion := schema.Info.Version
	...
	fmt.Fprintln(out, "\t// QixVersion is the Qix API version enigma was generated for.")
	fmt.Fprintln(out, "\tQixVersion = \""+qixVersion+"\"")
	fmt.Fprintln(out, ")")
}

func isValidVersion(version string) bool {
	// version should be d.d.d where d is an int
...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Println("Version required, please specify a version!")
		fmt.Println("Use: go run generate.go 'A.B.C'")
		os.Exit(1)
	}
	generateVersionFile(schema, os.Args[1], pwd)
